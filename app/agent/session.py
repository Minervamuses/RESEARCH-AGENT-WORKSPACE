...
class ChatSession:
    ...
    def _build_conversation_archive_hint(self) -> SystemMessage:
        ...
        return SystemMessage(content=(
            ...
            "under this root through the bash tool; every bash call "
            ...
            "text. POSIX-shell-quote that escaped phrase as exactly one literal "
            "argument (for example with `shlex.quote`), keep `--` before it, and "
            "POSIX-shell-quote the canonical root as one literal path before "
            "appending `/*.json` outside the quote. Never interpolate either "
            "operand unquoted. Use exactly `grep -lF -- "
            "<shell-quoted-escaped-phrase> <shell-quoted-root>/*.json | head -n "
            "21`. If the 21st path exists, "
