...
citation 是內建 skill（engine 位於 `app/skills/citation/`，同一目錄既是 skill bundle 也是 `skills.citation` package）。`/citation` 啟用後持續生效，之後以自然語言操作；agent 透過 skill 專屬的 `citation_workflow` 工具驅動「搜尋 → 呈現候選 → 使用者選擇 → 批次解析 match → 依當前請求的保存授權批次驗證+保存」。
...
- `citation_workflow` 是 skill 專屬工具：普通模式與其他 skill 綁不到、偽造呼叫會被執行層拒絕。同 session 並行 workflow 呼叫回 busy；`save`/`select`/`confirm` 可用 `identifiers` 一次帶最多 10 個 id，並在 busy lock 內依序執行。
- 保存意圖由模型直接理解當前自然語言請求；host/tool 不使用確認詞白名單或額外確認輪。請求已授權保存時使用單一 `save` action，以 candidate ids 在工具內完成 resolve+confirm；`select`/`confirm` 留給只檢視與多版本消歧。否定、條件、疑問或不明確語氣不得 save/confirm。
- 每個 candidate 的 DOI 解析採 0/1/多三分規則：0 個固定失敗、1 個由 `save` 立即保存、多個標示 `needs-disambiguation` 並保持 pending。既有 pending 與本次結果分區顯示，不能沿用本次授權順帶保存。
- Chat agent 使用 pinned `ChatOpenRouter`。select 後遇到空回應或 malformed tool call 時，graph 在剩餘 budget 內只做一次可呼叫工具的 continuation；再失敗才禁止工具 repair。診斷 log 只含 id、finish reason、token/count/budget，不含 DOI、工具參數或 provider 文字。
- 搜尋支援日期條件：`published_within_years` 或 `year_from`/`year_to`（互斥）；provider 用原生日期 filter，回傳後再做 fail-closed 年份篩選（年份未知或超出範圍一律剔除）。
- 搜尋輸出會明示實際日期窗；模型必須照實轉述。不同 DOI 的 preprint／正式版／reprint 只做非破壞式版本群組，shortlist 顯示代表版本但每個 `cX` 仍可獨立選擇。
- discovery 預設使用 RRF 加 bounded lexical relevance；可設 `CITATION_RANKING_MODE=rrf` 回退。版本化 venue catalog 平時只提供分類標示，僅在使用者明確要求 tier 時由 `refine` fail-closed 篩選，不參與一般排序。
...
save/confirm 回傳批次 artifact；finalizer 逐筆驗證成功 receipt，失敗只以穩定 reason code 對應固定文案。select 與多版本 save 另帶 DOI-free pending notes；若模型再次失敗，finalizer 會顯示仍有效的 `cX → mX` 與「尚未保存」。優先序固定為 confirm/save outcome → pending recovery → 模型文字/fallback。
