...
| 變數 | 必要性 | 用途 |
|---|---|---|
| `OPENROUTER_API_KEY` | chat agent、extended thinking、repo/folder ingest 必要 | OpenRouter chat model、RAG folder tagging |
...
| `DATACITE_MAILTO` | 選用 | citation skill 呼叫 DataCite 時加到 User-Agent，提高可用配額並保留聯絡方式 |
| `OPENALEX_API_KEY` | 選用 | 啟用 OpenAlex discovery；key 不進 cache、log 或 trace |
...
- Web Search MCP、GitHub MCP、citation Crossref/DataCite/OpenAlex/doi.org/arXiv authority 查詢都需要網路。
...
MCP server 如果啟動失敗，agent 仍會啟動，只是少掉那組外部工具。既有 Web Search/GitHub 的 legacy sanitizer 會把 stderr log 放在：

```text
~/.cache/agent-mcp/
```

Drop-in MCP 走 direct argv，不經 shell sanitizer；server 自己寫到 stderr 的內容目前可能直接出現在終端。
...
citation 是內建 skill。`citation_workflow` 的 stateless `search` 回傳完整 metadata 與穩定 DOI/arXiv identifier；`save` 接受 1–10 個由 agent 根據可見對話選定的自足 WorkIntent，不使用 candidate/match ID。
...
- 啟用取代目前 active skill；停用/切換 skill 會清除來源 registry（磁碟上的 bundle 保留）。citation active 期間 `/thinking extended` 被拒絕。
- Actions 只有 `search`、`save`、`sources`、`source`、`explain`；普通模式與其他 skill 的偽造呼叫仍會被拒絕。
- 作品、版本與寫入授權由 agent 依完整對話理解；不要求使用者在當前一句重複 `published`/`preprint`，host 也不再用 regex 或 binder 二次裁決。
- 同輪可有多次 save；workflow 會序列化執行，agent 可依前一次真實結果修正或重試。每次 save 的逐項結果與 receipt 都直接回傳給模型。
- DOI 經 doi.org CSL/BibTeX 重驗；v2 canonical identity bundle 原子保存，v1 bundle 只驗證重用。
- `search` 是自然語言探索，支援 `year_from`/`year_to`；不要自行拼 provider API 語法。模型可依對話中的順位指稱選擇，但 save 時應提交該結果顯示的 metadata/identifier。
- `save` 的 title/authors/year/venue/type/version/identifier 分欄；workflow 會分別產生 Crossref、DataCite、OpenAlex query，並採用符合模型 selection 的最佳記錄。
- 明確 DOI/arXiv 走 exact authority，不先 fuzzy search；工具負責查證 metadata 與寫入完整性，不重新判斷使用者是不是「真的」要該版本。
- citation active 時，registry-backed source marker 由 renderer 編號並產生 bibliography；raw DOI、數字、作者年份與手寫 bibliography 不再被格式 gate 封鎖。未知/dangling marker 仍會被拒絕。

save 在 tool content 回傳逐項 success/reused/ambiguity/not-found/failure 與真實 receipt，同時保留 strict batch artifact 供 telemetry。finalizer 不再以 deterministic save prose 覆蓋 agent 回答。
