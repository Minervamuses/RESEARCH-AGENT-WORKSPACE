...
- `citation_workflow` 是 skill 專屬工具：普通模式與其他 skill 綁不到、偽造呼叫會被執行層拒絕。同 session 並行 workflow 呼叫回 busy；`select`/`confirm` 可用 `identifiers` 一次帶最多 10 個 id，並在 busy lock 內依序執行。
- 保存意圖由模型直接理解當前自然語言請求；host/tool 不使用確認詞白名單或額外確認輪。請求已授權保存時可在同 turn `select` 後直接 `confirm`；否定、條件、疑問或不明確語氣不得 confirm。工具仍驗證 live match，書目一致性、atomic storage 與 receipt 驗證不受影響。
- 每個 candidate 的 DOI 解析採 0/1/多三分規則：0 個回報失敗、1 個可依授權直接保存、多個標示 `needs-disambiguation` 並要求明確 `mX`（除非使用者要求全部版本）。既有 pending 與本次 select 結果分區顯示，不能沿用本次授權自動 confirm。
...
confirm 每次回傳一個批次 artifact；finalizer 逐筆驗證成功 receipt，失敗只以穩定 reason code 對應固定文案。部分失敗與全數失敗都會取代模型草稿確定性顯示，不會靜默遮蔽。
