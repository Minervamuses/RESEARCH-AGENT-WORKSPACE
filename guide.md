...
- `search` 是自然語言探索，支援 `year_from`/`year_to`；不要自行拼 provider API 語法，結果順位不具保存語意。
- `save` 的 title/authors/year/venue/type/identifier 必須分欄；workflow 會分別產生 Crossref、DataCite、OpenAlex 的 strict→conditional fallback queries。
- 明確 DOI/arXiv 走 exact authority，不先 fuzzy search；provider 第一名、score、OpenAlex top-level DOI/primary location 均不能自行決定引用版本。
