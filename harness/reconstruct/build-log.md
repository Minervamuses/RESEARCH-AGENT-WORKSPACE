...
## 2026-09-05 18:32 CST — User-directed reconstruction correction preflight

- **Status:** correction pass `Not started` → `In progress`; this checkpoint records the required read-only gate before tests or implementation edits.
- **Runtime/ownership gate:** root=`/home/minervamuses/research-agent-workspace`, branch=`GUI`, start HEAD=`ff21796249a1b687f1638ddd546777027faa3c81`, upstream=`origin/GUI`, worktree clean (including untracked files). Commands and validation use WSL/Linux with Conda `app`; observed Python=`3.13.14`, Poetry=`2.3.4`, Node=`24.18.0`, npm=`11.16.0`, Cargo/Rust=`1.97.1`. The Windows download attachment is read-only input; repository execution remains Linux-native.
- **Decision authority read:** applicable root `AGENTS.md`; then the inherited `harness/plans/2026-08-24-desktop-gui-completion/` → `harness/fix_plans/` → `harness/reconstruct/` GOALS/PLANS/PROMPTS, relevant phases, and actual build logs. `harness/fix_plans/user-decisions.md` was read in full, including 002, 004, 007–009, and 012–014. The 2026-09-05 attachment supersedes only fixed complete-answer/file/wire/transcript capacity rejection and legacy conversation import requirements.
- **Confirmed correction scope:** canonical conversation models/repository and the canonical chunked-reader description; Python desktop service/protocol/server; protocol v1 contract; Rust child transport/validator; TypeScript validator and React transcript reconciliation; the isolated desktop fixture; focused tests; directly affected current README/`for_agents` descriptions; and current reconstruct GOALS/PLANS/PROMPTS plus this append-only log. Importer-only modules/tests/fixture branches may be removed after reference confirmation.
- **Preserved boundaries:** no Product Plan Mode restoration; no conversation-history Chroma or legacy Plan import; no changes to document-RAG Chroma/Ollama; no Fusion/Extended Thinking repair or restart-mode persistence; no Citation redesign; Normal remains authoritative `streamKind=final_only`, `chunkCount=0`, with no `answer.chunk`, preview, live-token, or simulated streaming. Input submission, tool/event/failure/identifier/path/SafeContent/approval/schema/UTF-8 protections remain bounded where they are not complete persisted conversation text.
- **Not authorized/not run:** no dependency or lockfile changes, live provider/Ollama/MCP calls, real user-store access or migration, commit, push, branch/worktree mutation, destructive Git operation, build, or test. Existing historical phases and evidence remain append-only history.
- **Initial source trace:** legal answer text is blocked at canonical 2 MiB, document 8 MiB, Desktop answer 2 MiB/transcript 32 KiB/page 1 MiB, protocol result 2 MiB/transcript 32 KiB, and Rust stdout 2 MiB gates. Normal production import fallback exists only on selected catalog IDs whose JSON is missing; the current fixture also seeds Plan logs and optionally invokes a batch importer. Retry success appends a second React live record and increments the displayed count instead of reconciling the authoritative `(conversationId, turnId)`.

## 2026-09-05 18:39 CST — Correction capacity baseline Red

- **Focused command:** from `app/`, `PYTHONDONTWRITEBYTECODE=1 conda run -n app poetry run pytest` with the six new repository/read-file/Desktop answer/contract/transcript/server selectors.
- **Observed result:** exit 1, `6 failed, 1 warning in 0.64s`. Each failure identifies a current prohibited gate: canonical `assistantOutput` 2 MiB, canonical read-file total 8 MiB, Desktop answer 2 MiB, protocol `maxBytes`, transcript degradation, and a complete >2 MiB result rejected before stdout delivery. A corrected server-only rerun also failed (`1 failed, 1 warning in 0.21s`) with an `ok=false` terminal result, isolating the wire rejection from fixture shutdown behavior.
- **Control note:** the first non-interactive invocation could not find `conda` because this host defines it through interactive shell initialization; no tests executed in that attempt. Subsequent Linux commands use the already-verified interactive WSL shell and Conda `app` environment.
- **Scope:** no production source, dependency, lockfile, user data, provider, Ollama, or migration was changed or invoked before recording this Red baseline.

## 2026-09-05 19:26 CST — Reconstruction correction Green and scope review

- **Capacity correction:** completed assistant text and already-accepted canonical input now retain strict type/nonblank/UTF-8/schema validation without an application-defined answer or whole-document byte ceiling. Canonical files read to EOF, and the approval-gated `read_file` path still returns at most 1 MiB per call while allowing an exact canonical UUID JSON file to be reconstructed through `next_offset` until EOF. The 1 MiB submit-input limit and tool, event, failure, identifier, path, schema, URL, approval, SafeContent, and raw-provider/tool bounds remain unchanged.
- **Final-only transport:** `session.turn` and `session.transcript` are the only explicitly method-correlated successful results allowed past the ordinary 2 MiB protocol-line boundary. Python validates the complete result before encoding; Rust first reads only a bounded prefix and continues only for the exact canonical Python success prefix matching the current generation, a pending request ID, and one of those two methods. It then still validates UTF-8, strict JSON, Python origin, full method schema, generation, and request correlation. Normal answers remain one terminal `streamKind=final_only`, `chunkCount=0` result with no answer event or preview.
- **Legacy removal:** deleted the three legacy conversation importer modules and importer-only test modules. Production create/list/select/restore/restart/continue and the fixture no longer read or generate Plan logs or conversation Chroma. Existing legacy source bytes remain untouched; a catalog-only ID with no canonical JSON is unavailable, creates no fake JSON/transcript, and cannot replace the selected healthy session.
- **Retry correction:** React reconciles restored and live turns by `(sessionId, turnId)`, orders by canonical `turnNumber`, lets the authoritative live result replace the failed/interrupted restored card, and computes count with the returned canonical number. Fixture coverage proves explicit failed-turn retry and completed replay reuse one ID/number/result without a duplicate provider side effect or sidebar count.
- **Regression evidence:** repository round-trip covers four answers individually beyond the former 2 MiB limit and one JSON document beyond the former 8 MiB limit, with Unicode/newline/quote/backslash preservation and exact latest-context text. Other focused Python selectors covered large final-only server output/transcript, 50-turn paging, bounded tool activity pages above the former aggregate page threshold, final persistence failure, A→B→A/restart, catalog-only selection, canonical-only fixtures, and legacy-module absence. Focused TypeScript/React tests covered large schema values, reducer dedupe/count/order, and multi-megabyte SafeContent rendering; focused Rust tests covered large results and bounded oversized ordinary/malformed/event/failure framing.
- **Review:** an independent read-only review found one P2 in the first Rust implementation: ordinary stdout was rejected only after an unbounded `read_until`. The focused correction restored fail-fast bounded-prefix framing, added no-newline/event/failure/malformed regressions, and retained the two correlated full-text exceptions. A fresh re-review found no remaining P1/P2 and confirmed no Fusion, Extended Thinking, or Citation production change.

## 2026-09-05 19:26 CST — Correction final validation and native attempt

- **Python suite:** the one final broad invocation collected 947 tests and finished `946 passed, 1 failed, 1 warning in 29.58s`. The sole failure was a stale assertion in `test_desktop_protocol_contract.py` that still required the intentionally removed `assistantText.maxBytes=32768`; production behavior and the other 946 tests passed. That assertion was corrected, and its exact selector then passed (`1 passed, 1 warning in 0.16s`). Per the once-only broad-suite rule, the already-passing 946 tests were not rerun. The warning is the existing LangGraph `allowed_objects` pending-deprecation.
- **TypeScript suite:** `conda run -n app npm test` passed `142/142` with no failed, skipped, or cancelled tests in `25375.289954ms`. Existing non-fatal Vite WebSocket messages reported port 24678 already in use during several SafeContent/app-helper tests.
- **Rust suite:** `conda run -n app cargo test --manifest-path src-tauri/Cargo.toml` passed all 36 library tests; binary and doc targets each had zero tests and passed. This includes exact multi-megabyte turn/transcript stdout, final-only invariants, and bounded-prefix rejection before a no-newline child finishes sleeping.
- **Tauri no-bundle:** `conda run -n app npm run tauri -- build --no-bundle` exited 0. Its `beforeBuildCommand` ran `tsc --noEmit` and Vite (`25 modules transformed`, built in 143 ms); Rust release finished in 1m12s and produced `app/desktop/src-tauri/target/release/research-agent-desktop`. No installer or bundle was created.
- **Native attempt:** launched that current release binary under Conda `app` with an owned direct root `/tmp/research-agent-desktop-correction-ZLHgWm`, mode 700, no legacy migration/crash opt-in, and the exact isolated fixture gate. The process remained alive, but no Python backend started before UI interaction. The available Computer Use inventory returned no native apps (`apps=[]`), so the WSLg window could not be targeted; no keyboard, retry, long-answer, exact `720×560`, or 200% native observation was possible. No browser/headless evidence was substituted. The process was interrupted, absence of desktop/backend processes was verified, and the empty synthetic root was deleted after exact realpath/parent/owner/non-symlink checks.
- **Documentation/static checks:** `git diff --check` passed. Targeted current-source searches found no importer symbols or removed conversation size constants outside the intentional negative module-absence test, and no answer-event producer. The external `manage_for_agents.py check` still reports 29 structural errors already present at start HEAD (missing two README headings, IDs repeated in the testing coverage table, and three pre-existing retired-ID bullet references); three errors introduced by the first documentation edit were corrected. Those unrelated baseline records were not broadly rewritten.
- **Scope and remaining evidence:** no dependency, lockfile, environment definition, Fusion/Extended Thinking/Citation production path, real user store, legacy user data, live provider, Ollama, MCP/network call, commit, push, branch, or worktree was changed or invoked. Automated correction evidence is complete; the current correction's native GUI journey remains unavailable rather than passed.
