...
Phase06 observed route correction（2026-09-13）：唯一完整 Rust run 為35 passed／1 failed；新 generation shutdown-report test 的 kill-only fixture 讓 stdout EOF 與 process exit 競速，可能先得到 Degraded。先保留 red，再只同步該既有測試的 kill＋wait（持有既有 Child mutex），維持 Crashed 與 NotRunning 原斷言，不修改 production lifecycle。只補跑該 targeted test，完整套件不重跑；若仍失敗依嘗試上限停止查因，不擴大實作。Tauri build 仍各一次。
