...
依使用者「跳過需要重啟的部分，如實記錄即可」的決定，採用 [GOALS 的驗收範圍調整](GOALS.md#2026-09-12-使用者調整本次驗收範圍)。Phase 03 可在已完成可用檢查、明列需要 WSLg 恢復的未驗證項、更新文件及清理本次暫存資料後，以有限驗收結束。這取代前文要求所有原生重啟檢查通過才能交付的 gate；不把 skipped 改成 pass，也不宣告原始完整矩陣全數通過。

Phase 01／02 的完成證據不變。不執行整個 WSL／Windows 重啟，不再為此次交付追加原生驗收；未來重驗由使用者另行啟動。
