...
## 2026-09-12 使用者調整本次驗收範圍

使用者在得知 WSLg 核心等待無法局部復原、整個 WSL 重啟會影響其他工作後，明確要求：「跳過需要重啟的部分，如實記錄即可」。本次交付因此不再要求重啟 WSL 後繼續驗收。

已通過的原生組字、候選／組字確認零送出、後續單次送出、精確中文保存及曾完成的 Desktop 重啟讀回仍有效。故障後必須恢復 WSLg 才能執行的新組字、完整目標句切換驗收、英文／中英文標點及 Shift+Enter 原生回歸，均記為「未驗證／使用者授權略過」，不視為通過。原成功條件保留作為完整驗收的參考；這些略過項不再阻止本次有限範圍交付。逐項證據見 build-log.md。
