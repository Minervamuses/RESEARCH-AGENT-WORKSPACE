...
| `cd app && conda run -n app poetry run pytest tests/test_desktop_protocol_contract.py tests/test_desktop_service.py tests/test_slash_commands.py tests/test_desktop_conversations.py tests/test_desktop_answer_stream.py tests/test_history_rag_store.py tests/test_session_eviction.py tests/test_plan_mode.py -q` | Desktop protocol, conversation, restore, stream, and control boundary | Completed GUI Phase 02 verification | Installed `app` environment; fake/temp state | Focused cross-subsystem result |
...
|---|---|---|---|
| Agent graph/session/turn lifecycle | Relevant test_state, test_graph_skill_loader, test_turn_finalizer, test_session_eviction, test_plan_mode modules | Complete pytest once near completion when inexpensive | No live provider unless explicitly approved |
...
| Desktop protocol/backend/conversations | Relevant Python desktop, history, plan, Bash modules plus `npm test` | Cargo test and selected Python regression; Tauri no-bundle build when native/build surfaces change | Isolated real Tauri/Python journey for lifecycle/UI changes; never use real user stores |
...
| INV-005 and INV-006 | test_turn_finalizer.py, test_thinking_session.py, test_plan_mode.py | Strong offline coverage; raised-provider exception paths remain |
...
| INV-016 | Conversation/history/plan/eviction tests, including A→B→A, restart, flush failure, and non-repersisting restores | Strong offline coverage; abnormal process loss after a finalized first turn remains uncovered |
...
- Default app/store, cite, plan_logs, extension state, MCP logs, node_modules, dist, and Rust target are generated/local. Tests should isolate write roots and avoid user data.
...
- Per-conversation mode/thinking/skill snapshots are tested in-process and intentionally reset to safe defaults after backend restart because current durable formats do not store them.
