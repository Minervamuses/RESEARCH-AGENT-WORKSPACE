...
- Confirmed by current Phase 07 offline evidence: batch migration (`7 passed`), six real Python-backend SIGKILL/restart checkpoints (`6 passed`), repository temporary-write faults, restart control reset, the no-`chat_history` fixture assertion, full Python/npm/Cargo suites, and the Tauri no-bundle release build. The production Tauri window also passed the keyboard, pending/final-only, retry, A→B→A, restart, mode, tool-separation, inert-content, focus, and scroll observations. Exact native `720×560` and 200% zoom layout evidence remains unavailable/not passed and blocks Phase 07 completion; see `testing-strategy.md`.
