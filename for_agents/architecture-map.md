...
- Unknown: resource behavior for a very large legacy Chroma migration tree and long-lived canonical stores approaching their explicit turn/document/file-scan limits.
...
- Confirmed by `harness/reconstruct/build-log.md` Phase 07 evidence: batch migration (`7 passed`), six real Python-backend SIGKILL/restart checkpoints (`6 passed`), repository temporary-write faults, restart control reset, the no-`chat_history` fixture assertion, full Python/npm/Cargo suites, and the Tauri no-bundle release build. The production Tauri window also passed keyboard, pending/final-only, retry, A-to-B-to-A, restart, mode, tool-separation, inert-content, focus, and scroll observations. Exact native `720x560` and 200% zoom layout evidence remains unavailable/not passed, so that plan stays In progress. The separate corrective plan remains Blocked under its own recorded broad-rerun authority; plan status does not override current code or later test evidence.
