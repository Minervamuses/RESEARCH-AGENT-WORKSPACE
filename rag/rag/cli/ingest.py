...
def _has_do_not_index_sentinel(path: Path, scan_lines: int = 8) -> bool:
...
def _collect_folders(
    ...
    for file_path in sorted(root.rglob("*")):
        ...
        if not _should_ingest(file_path):
            continue
        if file_path.suffix.lower() == ".md" and _has_do_not_index_sentinel(file_path):
            continue
...
def ingest_single(
    ...
    if _has_do_not_index_sentinel(path):
