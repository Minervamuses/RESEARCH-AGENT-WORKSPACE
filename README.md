...
| `/citation`(citation skill) | `app` env + 網路(Crossref/DataCite/doi.org);OpenAlex 需 `OPENALEX_API_KEY`;Web Search 預設可用於 fallback / `more`,OpenRouter query expansion 為選配 |
...
`search` 可帶 `year_from`/`year_to`，逐次查詢 Crossref、DataCite 與可用的 OpenAlex；`save` 內使用 bounded deterministic queries 並以 doi.org CSL/BibTeX 重新驗證。無 DOI 的作品若命中 trusted authority——WorkIntent 帶 arXiv ID(經 export.arxiv.org 取回 metadata),或內建 venue allowlist 中的記錄——仍可保存。`source` 只接受 stable `source_id`。
