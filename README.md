...
- workspace 根目錄的 `cite/`(預設;bundle 納入 git),或 `CITATION_OUTPUT_DIR` / `AgentConfig.citation_output_dir` 指向的位置:citation bundle 輸出(`<title>--<doi-hash>/reference.bib` + `citation.json`)。只有 wheel 安裝且 cwd/package 都不在 git workspace 時才 fallback 到平台 user-data 目錄。
...
`search` 可帶 `year_from`/`year_to`，逐次查詢 Crossref、DataCite 與可用的 OpenAlex；`save` 內使用 bounded deterministic queries 並以 doi.org CSL/BibTeX 重新驗證。`source` 只接受 stable `source_id`。
...
- **Discovery/Resolution**:search stateless；save 以 deterministic query 跨 Crossref/DataCite/OpenAlex fresh resolve，hard constraint 先 veto、score 只排序。
- **驗證**:DOI winner 必須由 doi.org refetch，BibTeX 經 pybtex canonical round-trip，identity-critical 衝突一律零寫入。
